...
import { Table, Form, Input, Button, Popconfirm, message, Space, notification,Upload ,Modal,Layout} from "antd";
        ...
        return (
          <Space>
            ...
            <Popconfirm 
              ...
              onConfirm={() => {
                const { _id } = record;
                axios.delete('http://localhost:7000/category/' + _id).then((response) => {
      ...
      <Layout.Content style={{ padding: 24 }}>
        ...
        >
          <Form
            ...
            onFinish={(values) => {
              // SUBMIT
              axios.patch('http://localhost:7000/category/' + selectedRow._id, values).then((response) => {
                if (response.status === 200) {
                  setRefresh((f) => f + 1);
                  setVisible(false);
                }
